...
function bumpVersion(release) {
  ...
  return fs.readJson("package.json").then((data) => {
...
function lint() {
  const filesToLint = ["gulpfile.js", "src/index.html", "src/inline-scripts/*"];
  const config = {
    useEslintrc: true,
  };
  return gulp
...
function generateServiceWorker() {
  return workbox
    .generateSW({
      ...
      globPatterns: ["**/*.{html,js,png,jpg,gif,ico}"],
...
function buildJS() {
  return gulp
    .src(`${SRC_DIR}/inline-scripts/*.js`)
    .pipe(terser(TERSER_OPTS))
    .pipe(gulp.dest(`${TEMP_DIR}/inline-scripts`));
}

function copyHTML() {
  const filesToCopy = [`${SRC_DIR}/index.html`];
  return gulp.src(filesToCopy).pipe(copy(TEMP_DIR, { prefix: 1 }));
}
...
function copyStatic() {
  ...
  return gulp.src(filesToCopy).pipe(copy(DEST_DIR, { prefix: 1 }));
}
